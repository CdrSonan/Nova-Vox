...
def loopSamplerVoicedExcitation(inputTensor, targetSize, repetititionSpacing, pitch, device):
    ...
    requiredTensors = max(math.ceil((targetSize/global_consts.batchSize - batchRS) / (inputTensor.size()[1] - math.ceil(global_consts.tripleBatchSize / pitch) - batchRS)), 1) #!!!BUGGED!!!

    #fix insertion timings, include modulo operations
    #pre-calculate istft
    #adjust repetitition spacing to only cover 1/2 of the shortest possible sample
