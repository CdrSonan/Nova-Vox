...
class SpecCrfAi(nn.Module):
    ...
    def forward(self, spectrum1:torch.Tensor, spectrum2:torch.Tensor, spectrum3:torch.Tensor, spectrum4:torch.Tensor, embedding1:torch.Tensor, embedding2:torch.Tensor, factor:torch.Tensor) -> torch.Tensor:
        ...
        embedding = torch.cat((factor[:, :, :global_consts.halfTripleBatchSize + 1 - 64], torch.tile(embedding1[None, :], (outputSize, 1, 2)), torch.tile(embedding2[None, :], (outputSize, 1, 2))), dim = 2)
        ...
        embedding = torch.cat((1. - factor[:, :, :global_consts.halfTripleBatchSize + 1 - 64], torch.tile(embedding1[None, :], (outputSize, 1, 2)), torch.tile(embedding2[None, :], (outputSize, 1, 2))), dim = 2)
