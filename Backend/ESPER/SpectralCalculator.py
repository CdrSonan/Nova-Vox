...
def calculateSpectra(audioSample:AudioSample, useVariance:bool = True) -> None:
    ...
    print(torch.any(torch.isnan(lowSpectra)))
    print(torch.any(torch.isnan(highSpectra)))
    audioSample = finalizeSpectra(audioSample, lowSpectra, highSpectra)
    audioSample = separateVoicedUnvoiced(audioSample)
    print(torch.any(torch.isnan(audioSample.excitation)))
    print(torch.any(torch.isnan(audioSample.specharm)))
    print(torch.any(torch.isinf(audioSample.excitation)))
    print(torch.any(torch.isinf(audioSample.specharm)))
    print(torch.any(torch.isneginf(audioSample.specharm)))
